import { useEffect, useState, useMemo, useCallback } from "react"; // 1. Tambah useCallback
...
const Reports = () => {
  ...
  const fetchReportData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get<{ data: Transaction[]; nextCursor: string | null }>(
        "/transactions",
      );
      setTransactions(res.data.data);
    } catch (error) {
      toast.error(toErrorMessage(error, "Gagal sinkronisasi laporan"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const reportStats = useMemo(() => {
    const filtered = transactions.filter(
      (txn) => toCalendarDay(txn.date) === selectedDate,
    );
    ...
    const totalExpense = filtered.reduce((total, txn) => total + txn.amount, 0);
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-44 font-sans overflow-x-hidden">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-24 bg-zinc-900/80 backdrop-blur-3xl border-t border-white/5 rounded-t-[2.5rem] flex justify-between items-center px-12 z-50 pb-6 transform-gpu shadow-2xl">
        ...
        <button
          ...
          className="flex flex-col items-center text-emerald-500 active:scale-90 transition-transform"
        >
