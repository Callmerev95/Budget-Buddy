...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // redirectTo dibangun dari origin saat ini, bukan localhost yang
      // di-hardcode seperti versi sebelumnya.
    ...
    <div className="min-h-screen bg-[#050505] flex flex-col justify-center px-8 py-12 relative overflow-hidden font-sans">
      ...
      >
            ...
            {!isSubmitted ? (
              ...
              >
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.25em] text-zinc-500 ml-2">
                    Registered Email
                  </label>
        ...
        <motion.div className="text-center mt-10">
          <Link
            to="/login"
            className="text-zinc-500 text-xs font-bold uppercase tracking-widest hover:text-emerald-500 transition-colors"
          >
            Back to Login
          </Link>
        </motion.div>
