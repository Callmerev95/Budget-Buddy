...
export const QuickActions = memo(
  ({
    ...
    return (
      <section className="-mt-4 transform-gpu">
        {" "}
        {/* Pastikan GPU layer aktif */}
        <div
          className={`backdrop-blur-2xl border rounded-[2.25rem] p-4 flex items-center transform-gpu transition-all duration-500 ${
            isDarkMode
          ...
          >
            <div
              className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                totalFixed > 0
                  ? isDarkMode
                    ? "bg-amber-500/10 text-amber-500 shadow-[inset_0_0_10px_rgba(245,158,11,0.1)]"
            ...
            <div className="text-left">
              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-0.5">
                Tagihan Bulanan
              </p>
              <p
              ...
              >
                {totalFixed > 0 ? formatCurrency(totalFixed) : "Belum Ada"}
              </p>
            </div>
            <ChevronRight
              size={12}
              className={`absolute top-0 right-0 transition-colors ${isDarkMode ? "text-zinc-700" : "text-zinc-300"}`}
            />
          </button>

          {/* Vertical Divider - Dinamis sesuai tema [cite: 2026-02-03] */}
          <div
            className={`w-[1px] h-8 mx-2 transition-all duration-500 bg-gradient-to-b ${
              isDarkMode
          ...
          <div className="flex-1 flex items-center gap-3 px-2">
            <div
              className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                monthlyBudgetFree < 0
                  ? isDarkMode
                    ? "bg-rose-500/10 text-rose-500 shadow-[inset_0_0_10px_rgba(244,63,94,0.1)]"
            ...
            <div className="text-left">
              ...
              >
                {formatCurrency(monthlyBudgetFree)}
              </p>
            </div>
          </div>
        </div>
