...
export const TransactionList = memo(
  ({ transactions, limit, onDelete, isDarkMode = true }: TransactionListProps) => {
        ...
        <div className="flex justify-between items-center mb-6 px-2">
          ...
          {limit && transactions.length > limit && (
            <motion.button
              ...
              className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest py-2 px-4 rounded-xl border transition-all duration-500 ${
                isDarkMode
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {displayedTransactions.length > 0 ? (
              displayedTransactions.map((item) => {
                ...
                return (
                  <motion.div
                    ...
                    layout
                    className={`p-4 rounded-[1.75rem] border flex items-center justify-between backdrop-blur-md relative overflow-hidden transform-gpu transition-all duration-500 ${
                      isDarkMode
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div
                        className={`w-12 h-12 flex-shrink-0 ${details.bg} ${details.color} rounded-2xl flex items-center justify-center shadow-sm transition-colors duration-500`}
                      >
                        {details.icon}
                      </div>
                      <div className="min-w-0">
                        <p
                          className={`text-sm font-black leading-tight tracking-tight truncate transition-colors duration-500 ${
                      ...
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <div
                          className={`flex items-center gap-1 transition-opacity duration-500 ${
                            isDarkMode ? "text-emerald-500/60" : "text-emerald-600/80"
                          }`}
                        >
                      ...
                      {onDelete && (
                        <motion.button
                          ...
                          className={`w-9 h-9 flex items-center justify-center transition-colors ${
                            isDarkMode
                              ? "text-zinc-700 active:text-rose-500"
                              : "text-zinc-300 active:text-rose-500 hover:text-zinc-400"
                          }`}
                        >
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className={`border border-dashed rounded-[2.5rem] py-16 text-center w-full transition-all duration-500 ${
                  isDarkMode
              ...
              >
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border transition-all duration-500 ${
                    isDarkMode
