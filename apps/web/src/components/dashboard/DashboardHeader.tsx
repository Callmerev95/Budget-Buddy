...
export const DashboardHeader = memo(
  ({
    ...
    const timeContext = useMemo(() => {
      ...
      if (hour >= 5 && hour < 11)
        return { label: "Pagi", icon: <Sunrise className="text-amber-400" size={20} /> };
      if (hour >= 11 && hour < 15)
        return { label: "Siang", icon: <Coffee className="text-orange-400" size={20} /> };
      if (hour >= 15 && hour < 18)
        return { label: "Sore", icon: <CloudSun className="text-rose-400" size={20} /> };
    ...
    const themeClasses = {
      header: scrolled
        ? isDarkMode
          ? "bg-[#050505]/80 backdrop-blur-xl shadow-2xl border-b border-white/5"
          : "bg-white/80 backdrop-blur-xl shadow-lg border-b border-zinc-200"
        : "bg-transparent",
      card: isDarkMode
        ? "bg-zinc-900/80 border-white/5 text-zinc-400"
        : "bg-zinc-100 border-zinc-200 text-zinc-600",
      title: isDarkMode ? "from-white to-zinc-500" : "from-zinc-900 to-zinc-500",
    };
