  ...
  >
          ...
          <RechartsPie>
            ...
            <Tooltip
              ...
              formatter={(value) => {
                ...
                const numValue = typeof raw === "string" ? Number(raw) : (raw ?? 0);
                return [
                  formatCurrency(Number.isFinite(numValue) ? numValue : 0),
                  "Total",
                ];
              }}
            />
    ...
    <div className="grid grid-cols-2 gap-3 mt-8">
      {chartData.map((item, index) => (
        <div
          key={index}
          className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5 active:scale-95 transition-all"
        >
          <div
            className="w-2.5 h-2.5 rounded-full shadow-[0_0_8px_inherit]"
            style={{ backgroundColor: COLORS[index % COLORS.length] }}
          />
          <div className="flex-1 min-w-0">
            <p className="text-[9px] font-black text-zinc-400 uppercase truncate">
              {item.name}
            </p>
